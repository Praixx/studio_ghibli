...
function addCard (array){
    array.forEach( eachItems =>{
        createCard(eachItems);
    })
}
...
function filmCardContent (data){
    ...
    html += `<p><strong>Director: ${data.director}</strong></p>`;
    html += `<p><strong>Released: ${data.release_date} </strong></p>`;
    html += `<p><strong>Rating: ${data.rt_score} </strong></p>`;
...
async function peopleCardContent (data){
    ...
    for( eachFilm of theFilms){
        const filmTitle = await individualItems(eachFilm, "title");
        filmTitles.push(filmTitle); 
    }

    const species = await individualItems(data.species, "name");
...
async function locationCardContent(data){
    ...
    const theResident = data.residents;
    let residentNames = [];
    for(eachResident of theResident){
       if(eachResident.match(regex)){
            const resName = await individualItems(eachResident, "name");
...
async function speciesCardContent(data){
    ...
    for(eachperson of people ){
        const personName = await individualItems(eachperson, "name");
    ...
    let html = `<h2>${data.name} </h2>`;
    html += `<p><strong>Classificstion:</strong> ${data.classification}</p>`;
...
async function vehicleCardContent(data){
    ...
    html += `<p><strong>Vehicle:</strong> ${data.vehicle_class}</p>`;
    html += `<p><strong>Length:</strong> ${data.length}</p>`;
    html += `<p><strong>Pilot:</strong> ${await individualItems(data.pilot, "name")}</p>`;
    html += `<p><strong>Film:</strong> ${await individualItems(data.films, "title")}</p>`;
    return html;
}



async function individualItems (url, item){
    var theItem
